# randomQuizGenerator.py - Creates qiuzzes with questions and answers
# in random order, along with the answer key

import random

# The quiz data. Keys are status and values are the capitals
...
for quizNum in range(35):
	...
	for questionNum in range(50):
		...
		corrrectAnswer = capitals[states[questionNum]]
		wrongAnswers = list(capitals.values())
		del wrongAnswers[wrongAnswers.index(corrrectAnswer)]
		...
		answerKeyFile.write('%s. %s\n' % ((questionNum + 1), 
			'ABCD'[answerOptions.index(corrrectAnswer)]))
	quizFile.close()
	answerKeyFile.close()